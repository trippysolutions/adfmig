...
public final class HtmlReport {
    ...
    private void method(StringBuilder out, ApplicationAssessment a) {
        ...
        limits(out);
        out.append("<footer>Generated by adfmig ").append(escape(version()))
                .append(" &middot; ").append(LocalDate.now())
                .append(" &middot; estimates are uncalibrated pending a completed migration.<br>")
                .append("adfmig &middot; Trippy Solutions &middot; ")
                .append("<a href=\"https://trippysolutions.com\">trippysolutions.com</a></footer>\n");
    }

    /**
     * What the report cannot tell them, said plainly at the end.
     *
     * <p>This report travels: it is one self-contained file and it gets forwarded to people who
     * will not run the tool themselves. Someone reading it having skipped the numbers should
     * still leave knowing which questions are still open, rather than believing a cost estimate
     * is a plan.
     */
    private void limits(StringBuilder out) {
        out.append("<h2>What this does not answer</h2>\n")
                .append("<ul>\n")
                .append("<li><b>The parts that need a person.</b> Anything the metadata does not ")
                .append("describe — a Groovy expression, hand-written SQL, custom Java — cannot be ")
                .append("translated mechanically. Each one is listed above with its location, and ")
                .append("each is someone's afternoon.</li>\n")
                .append("<li><b>What the estimate is worth.</b> Effort here is scored from what the ")
                .append("application contains, not measured against a migration that finished. It ")
                .append("ranks cost reliably and predicts it loosely.</li>\n")
                .append("<li><b>The order to do it in.</b> An application sharing a database or a ")
                .append("library with another cannot be migrated alone; run <code>adfmig apps</code> ")
                .append("across the whole estate to see what depends on what.</li>\n")
                .append("</ul>\n");
    }
